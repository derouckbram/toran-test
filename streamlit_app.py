        ...
        while True:
            ...
            for r in docs_json.get('resources', []):
                ...
                for f_item in fields_list:
                    ...
                    if any(kw in attr for kw in target_keys):
                ...
                if not due_date:
                    for f_item in fields_list:
                        ...
                        if re.match(r'^\d{4}-\d{2}-\d{2}$', val):
                            # Double check attribute name isn't a "Start" date
                            attr = str(f_item.get('attribute', '')).lower()
                            if any(bad in attr for bad in ['from', 'issue', 'start', 'begin', 'since', 'birth']):
                                continue
                            try: 
            ...
            # Turn to the next page and loop again
            page += 1 
