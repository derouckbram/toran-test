...
def fetch_and_merge_data_v2(end_date):
    ...
    if c_xsrf: 
        c_sess.headers.update({
            'X-XSRF-TOKEN': urllib.parse.unquote(c_xsrf),
            'X-Requested-With': 'XMLHttpRequest'
        })

    t_xsrf = t_sess.cookies.get('XSRF-TOKEN')
    if t_xsrf: 
        t_sess.headers.update({
            'X-XSRF-TOKEN': urllib.parse.unquote(t_xsrf),
            'Referer': 'https://admin.toran.be/planning', 
