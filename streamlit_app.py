...
def fetch_and_merge_data_v2(end_date):
    ...
    # 2. LAST PERFORMED MAINTENANCE (Using Endpoint from your Logs)
    ...
    if hist_json:
        ...
        for r in hist_json.get('resources', []):
            # Parse the Nova fields structure
            fields = {f['attribute']: f['value'] for f in r.get('fields', [])}
            
            # Find Registration
            reg_raw = str(fields.get('aircraft') or "")
            if isinstance(fields.get('aircraft'), dict): # Nova sometimes returns objects
                reg_raw = fields.get('aircraft', {}).get('display', "")
            
            reg_merge = normalize_tail(reg_raw.split(' ')[0])
            
            # Find Date (Scan common fields)
            date_val = None
            ...
            # Find Hours (Scan common fields)
            hist_hours = None
            for k in ['ttsn', 'hours', 'aircraft_hours', 'total_time', 'tacho', 'current_hours']:
                if fields.get(k):
                    try: hist_hours = float(str(fields.get(k)).replace(',', '')); break
                    except: pass

            # Find Type/Description
            m_type = str(fields.get('type') or fields.get('maintenance_type') or fields.get('name') or "Maintenance")
        ...
        if hist_list:
            ...
            # Keep only the first row (latest) for each aircraft
...
# ==========================================
# MODE 1: MAINTENANCE DASHBOARD (Full Restore + History)
# ==========================================
if app_mode == "Maintenance Dashboard":
    ...
    if df is not None:
        ...
        with tabs[0]:
            ...
            cols_to_show = ['Registration', 'Type', 'Current', 'Potential', 'Life Now %', 'Planned', 'Forecast', 'Life Forecast %', 'Due Date']
...
# ==========================================
# MODE 2: GUEST WELCOME SCREEN (Split Layout)
# ==========================================
